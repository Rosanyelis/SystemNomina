<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-h5 text-ink dark:text-dark-ink">{{ __('Nueva empresa') }}</h2>
    </x-slot>

    <x-ui.page-header
        :title="__('Nueva empresa')"
        :description="__('Registra una nueva empresa cliente en la plataforma.')"
    />
    ...
    <x-ui.card>
        <form method="POST" action="{{ route('empresas.store') }}" class="space-y-6">
            ...
            <x-ui.form-field>
                ...
                <x-ui.input id="razon_social" name="razon_social" type="text" :value="old('razon_social')" required autofocus />
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <x-ui.form-field>
                    ...
                    <x-ui.input id="rif" name="rif" type="text" :value="old('rif')" required placeholder="J-12345678-0" />
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <x-ui.form-field>
                    ...
                    <x-ui.input id="telefono" name="telefono" type="text" :value="old('telefono')" />
                ...
                <x-ui.form-field>
                    ...
                    <select id="activo" name="activo" class="block w-full rounded-md border border-border bg-surface text-ink shadow-ocmb-sm transition-colors duration-ocmb focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent dark:border-white/10 dark:bg-dark-surface dark:text-dark-ink h-10 min-h-11 px-3 py-2 text-sm md:min-h-10">
                        <option value="1" {{ old('activo', '1') == '1' ? 'selected' : '' }}>{{ __('Activa') }}</option>
                        <option value="0" {{ old('activo') == '0' ? 'selected' : '' }}>{{ __('Inactiva') }}</option>
                    </select>
            ...
            <x-ui.form-field>
                ...
                <textarea id="direccion" name="direccion" rows="3" class="block w-full rounded-md border border-border bg-surface text-ink shadow-ocmb-sm transition-colors duration-ocmb placeholder:text-ink-secondary focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent dark:border-white/10 dark:bg-dark-surface dark:text-dark-ink dark:placeholder:text-dark-ink/50 h-10 min-h-11 px-3 py-2 text-sm md:min-h-10">{{ old('direccion') }}</textarea>
